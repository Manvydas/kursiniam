...
fomula_prog=formula(mod_prog)

lagdata=data.frame()
for (i in 3:length(data4[1,])){
ccfvalues <- Find_Max_CCF2(data4$OMX,data4[,i],i)
  if(i==3){lagdata=ccfvalues}
  else{
  lagdata=rbind(lagdata,ccfvalues)
  }
...
mod2=lm(OMX~lag(SP350,lagdata[1,2])+lag(kk,lagdata[2,2])+lag(dll,lagdata[3,2])+lag(nedarbas,lagdata[4,2])+lag(mhope,lagdata[5,2])+lag(phope,lagdata[6,2])+lag(pramhope,lagdata[7,2])+lag(shope,lagdata[8,2])+lag(ul,lagdata[9,2])+lag(vhope,lagdata[10,2])+lag(vp,lagdata[11,2])+lag(mp,lagdata[12,2])+lag(gkl,lagdata[13,2])+lag(ip,lagdata[14,2])+lag(kursas,lagdata[15,2]),data=data4)

mod_kain = stepAIC(mod2, direction="both")

fomula_kain=formula(mod_kain)
...
colnames(lentele)=c("prad. imt. dyd.","strateg. graza","vid. random graza","tikimybe, kad geriau uz random","pirk. sk.")
kable(lentele)
...
validation2=function(pradzia){
  ...
  for(i in pradzia:168){ 
    ...
    a=a[length(a)]
      ...
      lagdata=data.frame()
      for (u in 3:length(data4[1,])){
        ccfvalues <- Find_Max_CCF2(data4$OMX,data4[1:i,u],u)
        if(u==3){lagdata=ccfvalues}
        else{lagdata=rbind(lagdata,ccfvalues)}
      }
    mod2=lm(OMX~lag(SP350,lagdata[1,2])+lag(kk,lagdata[2,2])+lag(dll,lagdata[3,2])+lag(nedarbas,lagdata[4,2])+lag(mhope,lagdata[5,2])+lag(phope,lagdata[6,2])+lag(pramhope,lagdata[7,2])+lag(shope,lagdata[8,2])+lag(ul,lagdata[9,2])+lag(vhope,lagdata[10,2])+lag(vp,lagdata[11,2])+lag(mp,lagdata[12,2])+lag(gkl,lagdata[13,2])+lag(ip,lagdata[14,2])+lag(kursas,lagdata[15,2]),data=data4[1:i,])
    modkain=stepAIC(mod2, direction="both")
    b=predict(modkain, data4[1:i,])
    b=b[length(b)]
    b= b-data4[i,"OMX"]
...
lentele2=data.frame()
for(i in c(100,110,120,130,140,150,160)){
  lentele2=rbind(lentele2,validation2(i))
}
...
length(prog)
length(a[,1])
a=cbind(prog,data4$OMX[102:length(data4$OMX)])
...
colnames(a)=c("menesiai","progn", "reali")
akc = melt(a, id=c("menesiai"))
ggplot(akc) + geom_point(aes(x=menesiai, y=value, colour=variable)) +geom_line(aes(x=menesiai, y=value, colour=variable))+
  scale_colour_manual(values=c(1:2))+ylab("% pokytis")+
  guides(col=guide_legend(title="Kintamieji")) +
  geom_hline(yintercept = 2)+
  geom_hline(yintercept = -2)
