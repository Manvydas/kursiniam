...
```{r, - infliacija,  0.147154->0.215864}
# KORELIACIJA:
names2=c("SP350","SP500","kk","dll","nedarbas","infliacija","mhope","phope","pramhope","shope","ta","ul","vhope","vp","mp","palukanos","gkl","ip","kursas")

for (i in 3:length(data3[1,])){
ccfvalues <- ccf(data3$infliacija,data3[,i]) #aiskinames laga
print(names2[i-2])
print(ccfvalues)
}

# KAS YRA PRADINIAM MODELY:
lag(SP350, 1) + lag(infliacija, 2) + lag(mhope, 3) + lag(phope,12) + lag(ul, 10) + lag(gkl, 9) + lag(ip, 4) + lag(kursas,2) 

# TEST KINT MODELIS:
mod_infliacija = lm( infliacija ~ lag(SP350,0)+ lag(kk,0) + lag(dll,0) + lag(nedarbas,0) + lag(mhope,0) + lag(phope,1) + lag(pramhope,0) + lag(shope,0) + lag(ul,0) + lag(vhope,0) + lag(vp,0) + lag(mp,0) + lag(gkl,5) + lag(ip,0)  + lag(kursas,0), data=data3)

stepAIC(mod_infliacija)

mod_po_aic_infliacija = lm(infliacija ~ lag(SP350, 0) + lag(kk, 0) + lag(nedarbas, 0) + lag(phope, 1) + lag(shope, 0) + lag(vhope, 0) + lag(gkl, 5) + lag(ip, 0) + lag(kursas, 0), data=data3)

mod_po_aic_infliacija_maxccf = lm(infliacija ~ lag(SP350, 12) + lag(nedarbas, 5) + lag(pramhope, 15) + lag(shope, 13) + lag(vhope, 15) + lag(vp, 15) + lag(gkl, 5) + lag(kursas, 9), data=data3)  # max ccf lagai

ncvTest(mod_po_aic_infliacija)
# Kadangi p-value > 0.05, tai H0 hipotezė priimama - duomenys homoskedastiški.

coeftest(mod_po_aic_infliacija, vcov=vcovHC)
# kk - 0.15965; phope - 0.14763;
summary(mod_po_aic_infliacija)
# pramhope - 0.10411, kai max ccf
...
```{r, - mazmenines prekybos pasitikejimas,  0.110307->0.395422}
# KORELIACIJA:
names2=c("SP350","SP500","kk","dll","nedarbas","infliacija","mhope","phope","pramhope","shope","ta","ul","vhope","vp","mp","palukanos","gkl","ip","kursas")
...
```{r, + paslaugu pasitikejimas, 0.071030->0.101939}
# KORELIACIJA:
names2=c("SP350","SP500","kk","dll","nedarbas","infliacija","mhope","phope","pramhope","shope","ta","ul","vhope","vp","mp","palukanos","gkl","ip","kursas")
...
# TEST KINT MODELIS:
mod_infliacija = lm( phope ~ lag(SP350,5)+ lag(kk,1) + lag(dll,0) + lag(nedarbas,0) + lag(infliacija,6) + lag(mhope,0) + lag(pramhope,1) + lag(shope,0) + lag(ul,0) + lag(ul,11) + lag(vhope,3) + lag(vp,0) + lag(mp,1) + lag(gkl,9) + lag(ip,1) + lag(kursas,0), data=data3)
 

stepAIC(mod_infliacija)

mod_po_aic_infliacija = lm(phope ~ lag(dll, 0) + lag(mhope, 0) + lag(pramhope, 1) + lag(shope, 0) + lag(ul, 11) + lag(mp, 1) + lag(gkl, 9) + lag(ip, 1) + lag(kursas, 0), data=data3)
...
modelis_po_aic <- lm(formula = OMX ~ lag(SP350, 1) + lag(infliacija, 2) + lag(mhope, 3) + lag(phope,12) + lag(ul, 10) + lag(gkl, 9) + lag(ip, 4) + lag(kursas,2) 
                     +
                      lag(dll, 12) + lag(pramhope, 13) + lag(shope, 12) + lag(mp, 13) # +12 lago ,nes phope uzlagintas 12
                     , data = data3)
...
```{r, +- uzsakimu lukesciai,  0.039495->0.151728}
# KORELIACIJA:
names2=c("SP350","SP500","kk","dll","nedarbas","infliacija","mhope","phope","pramhope","shope","ta","ul","vhope","vp","mp","palukanos","gkl","ip","kursas")
...
```{r, + gamintoju kainu lygis,  0.024197->0.076468}
# KORELIACIJA:
names2=c("SP350","SP500","kk","dll","nedarbas","infliacija","mhope","phope","pramhope","shope","ta","ul","vhope","vp","mp","palukanos","gkl","ip","kursas")
...
```{r, - industrine produkcija,  0.139994->0.47512}
# KORELIACIJA:
names2=c("SP350","SP500","kk","dll","nedarbas","infliacija","mhope","phope","pramhope","shope","ta","ul","vhope","vp","mp","palukanos","gkl","ip","kursas")
...
# kursas - 0.297466; infliacija - 0.254082; nedarbas - 0.273576
